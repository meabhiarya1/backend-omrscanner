...
const getImage = async (req, res, next) => {
  try {
    ...
    const sourceFileExists = await fs.access(sourceFilePath).then(() => true).catch(() => false);
    if (!sourceFileExists) {
    ...
    const allImages = await Promise.all(data.map(async (item) => {
      const { attribute, coordinateX, coordinateY, width, height } = item.dataValues;
